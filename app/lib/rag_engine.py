...
Previous conversation:""",
    ...
    def semantic_search(
        ...
        if context_paper_ids:
            placeholders = ",".join(
                [f":paper_id_{i}" for i in range(len(context_paper_ids))]
            )
    ...
    def answer_question(
        ...
        if context_paper_ids:
            ...
            search_results = self.semantic_search(
                ...
                context_paper_ids=(
                    context_paper_ids if len(context_paper_ids) <= 20 else None
                ),
            )
    ...
    def suggest_papers(
        ...
        return self.semantic_search(
            query=query, top_k=limit, min_year=min_year, max_year=max_year
        )
